  ...
  <div class="font-body bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 min-h-screen">
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-8 flex items-center justify-between">
        ...
        <button v-if="!isDetailView" @click="showCreateModal = true"
        ...
        <button v-else-if="listsStore.currentList" @click="showAddPlayerModal = true"
          class="bg-primary hover:bg-emerald-600 text-white font-bold py-3 px-6 rounded-lg transition-colors flex items-center gap-2">
      ...
      <div v-else-if="!isDetailView" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <div v-for="list in listsStore.lists" :key="list.id"
      ...
      <div v-else-if="isDetailView && listsStore.currentList"
        class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
        <div class="flex items-center justify-between mb-6">
          <div>
            <button @click="router.push('/mi-lista')"
              class="text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 mb-2 flex items-center gap-1">
        ...
        <div v-else class="mt-6">
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="item in listPlayers" :key="item.player.id"
              ...
              <div class="relative aspect-[3/4] overflow-hidden bg-slate-100 dark:bg-slate-800">
                <img v-if="item.player.photo" :src="item.player.photo" :alt="item.player.name"
    ...
    <div v-if="showCreateModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
      ...
      <div class="bg-white dark:bg-slate-900 rounded-xl shadow-xl p-6 w-full max-w-md">
        ...
        <input v-model="newListName" type="text" placeholder="Nombre de la lista (mínimo 3 caracteres)"
        ...
        <div class="flex gap-3 mt-6">
          ...
          <button @click="handleCreateList" :disabled="newListName.trim().length < 3"
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
      ...
      <div class="bg-white dark:bg-slate-900 rounded-xl shadow-xl p-6 w-full max-w-md">
        ...
        <p class="text-slate-600 dark:text-slate-400 mb-6">¿Estás seguro de que quieres eliminar esta lista? Esta acción
    ...
    <div v-if="showEditNameModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50"
      @click.self="showEditNameModal = false">
      <div class="bg-white dark:bg-slate-900 rounded-xl shadow-xl p-6 w-full max-w-md">
        <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-4">Editar nombre</h3>
        <input v-model="editName" type="text" placeholder="Nuevo nombre (mínimo 3 caracteres)"
          class="w-full px-4 py-3 border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent"
          @keyup.enter="confirmEditName" />
        <div class="flex gap-3 mt-6">
          <button @click="showEditNameModal = false"
            class="flex-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold py-2 px-4 rounded-lg transition-colors">
            Cancelar
          </button>
          <button @click="confirmEditName" :disabled="editName.trim().length < 3"
            class="flex-1 bg-primary hover:bg-emerald-600 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-semibold py-2 px-4 rounded-lg transition-colors">
            Guardar
          </button>
        </div>
    ...
    <ModalBuscarJugador :model-value="showAddPlayerModal" @update:model-value="showAddPlayerModal = $event"
